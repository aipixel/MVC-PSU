...
from dataset.panopli import PanopLiDataset, InconsistentPanopLiDataset, InconsistentPanopLiSingleDataset, create_segmentation_data_panopli, SegmentPanopLiDataset, CorresPanopLiDataset
...
def get_dataset(config, load_only_val=False):
    if config.dataset_class == "panopli":
        ...
        if not load_only_val:
            train_set = PanopLiDataset(Path(config.dataset_root), "train", (config.image_dim[0], config.image_dim[1]), config.max_depth, overfit=config.overfit, load_depth=True, 
                                       load_feat=config.use_feature_regularization, semantics_dir='m2f_semantics', instance_dir='m2f_instance', instance_to_semantic_key='m2f_instance_to_semantic',
...
def get_correspondence_dataset(config):
    if config.dataset_class == "panopli":
        return CorresPanopLiDataset(Path(config.dataset_root), "train", (config.image_dim[0], config.image_dim[1]), config.max_depth, overfit=config.overfit,
                                     max_rays=config.max_rays_segments, semantics_dir='m2f_semantics', instance_dir='m2f_instance', instance_to_semantic_key='m2f_instance_to_semantic',
                                     create_seg_data_func=create_segmentation_data_panopli, subsample_frames=config.subsample_frames)
    raise NotImplementedError