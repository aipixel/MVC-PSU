...
class BaseDataset(Dataset):
    ...
    def __init__(self, root_dir, split, image_dim, max_depth, overfit=False, num_val_samples=8, load_depth=False, load_feat=False, semantics_dir='filtered_pano_sem', instance_dir='filtered_pano_instance',
                 instance_to_semantic_key='instance_to_semantic', create_seg_data_func=create_segmentation_data_base, subsample_frames=1, run_setup_data=False):
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train":
            ...
            selected_rgbs = self.all_rgbs[idx]
            selected_semantics = self.all_semantics[idx]
            selected_instances = self.all_instances[idx]
            selected_probabilities = self.all_probabilities[idx]
            selected_confidences = self.all_confidences[idx]
            selected_depths = self.all_depths[idx] if self.load_depth else torch.zeros(1, 1)
            selected_feats = self.all_feats[idx] if self.load_feat else torch.zeros(1, 1)
            selected_pose = self.all_poses[idx]
            selected_masks = self.all_masks[idx]
            
            if selected_rays.shape[0] > 2048:
                sampled_indices = torch.randperm(selected_rays.shape[0])[:2048]
                selected_rays = selected_rays[sampled_indices, :8]
                selected_rgbs = selected_rgbs[sampled_indices, :]
                selected_semantics = selected_semantics[sampled_indices]
                selected_instances = selected_instances[sampled_indices]
                selected_probabilities = selected_probabilities[sampled_indices, :]
                selected_confidences = selected_confidences[sampled_indices]
                selected_masks = selected_masks[sampled_indices]
                if self.load_depth:
                    selected_depths = selected_depths[sampled_indices]
                if self.load_feat:
                    selected_feats = selected_feats[sampled_indices, :]
            ...
            image, rays, semantics, instances, depth, _, prob, conf, _, mask, _ = self.load_sample(sample_idx)
            sample = {
                "rays": rays,
                "rgbs": image,
                "semantics": semantics,
