    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(trajectory_loader)):
            ...
            for i in range(0, batch['rays'].shape[0], config.chunk):
                out_rgb_, out_semantics_, out_instances_, out_depth_, _ = renderer(model, batch['rays'][i: i + config.chunk], config.perturb, test_set.white_bg, False)
...
if __name__ == "__main__":
    ...
    print(Path(sys.argv[1]).parents[1])
    print(Path(sys.argv[1]).parents)
